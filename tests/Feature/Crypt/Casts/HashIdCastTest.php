...
test('it encodes a value correctly in get method', function () {
    $result = $this->hashIdCast->get($this->model, $this->key, $this->value, []);
    expect($result)->toBe($this->encodedValue);
});

test('it returns original value if hashids is disabled in get method', function () {
    Config::set('hashids.enable', false);
    $result = $this->hashIdCast->get($this->model, $this->key, $this->value, []);
    expect($result)->toBe($this->value);
});

test('it decodes a value correctly in set method', function () {
    $result = $this->hashIdCast->set($this->model, $this->key, $this->encodedValue, []);
    expect($result)->toBe($this->value);
});

test('it returns original value if hashids is disabled in set method', function () {
    Config::set('hashids.enable', false);
    $result = $this->hashIdCast->set($this->model, $this->key, $this->encodedValue, []);
    expect($result)->toBe($this->encodedValue);
});
